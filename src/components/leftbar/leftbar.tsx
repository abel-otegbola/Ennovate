...
function LeftBar() {
    ...
    return (
        ...
        <div className={`xl:w-[18%] lg:w-[22%] md:w-[27%] pb-[10px] h-screen md:sticky fixed top-[60px] p-2 left-0 bg-white dark:bg-black border border-transparent border-r-gray-200 dark:border-r-slate-100/[0.09] overflow-hidden z-10 transition-all duration-700 ${open ? " w-[240px]": "w-0"}`}>
            ...
            </div>
            {
                generalLinks.map(link => {return (
                    <a key={link.id} href={link.link} className={`flex items-center justify-between w-full p-1 my-[2px] px-4 rounded hover:bg-[#5938DD] hover:text-white ${pathname === link.link ? "bg-[#5938DD] text-white" : ""}`}>
